...
console.log("[runAllocatorProcess] Passo 3 Complete.");
 ...
 currentItemAllocations.forEach((alloc, i) => { // Added index 'i' to access itemsToProcess
     ...
     // Check for zero allocation for non-zero target after all steps
     const itemForAlloc = itemsToProcess[i];
     if (!alloc.error && itemForAlloc && itemForAlloc.amount > 0 && alloc.sum === 0 && (!alloc.combination || alloc.combination.length === 0)) {
        alloc.error = "Alocação zero para alvo > 0.";
        console.warn(`[runAllocationProcess] Passo Final: Item ${itemForAlloc.originalIndex + 1} ('${itemForAlloc.details}') - Alvo: ${itemForAlloc.amount}, Alocado: 0. Marcado como erro.`);
     }
 });
